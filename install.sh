...
need_root() {
  if [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
    ...
    echo "  curl -fsSL https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/${REPO_DEFAULT_REF}/install.sh | sudo bash"
    exit 1
  fi
}
...
usage() {
  cat <<'EOF'
Usage:
  sudo bash install.sh [--offline | --online | --latest-release]
...
download_script_offline() {
  ...
  err "Fix: place ${SCRIPT_NAME_IN_REPO} next to install.sh, or in /root."
  exit 1
}
